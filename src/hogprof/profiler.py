...
class Profiler:
    ...
    def __init__(
        ...
        if oma:
            ...
            self.taxaIndex, self.ReverseTaxaIndex = files_utils.generate_taxa_index(
                self.tree
            )
            self.treeweights = hashutils.generate_treeweights(
                self.tree, self.taxaIndex, None, None
            )
    ...
    def hogid2fam(self, hog_entry):
        if type(hog_entry) == int:
            ...
            hog_entry = self.db_obj.entry_by_entry_nr(
                self.db_obj.id_resolver.resolve(hog_entry)
            )
    ...
    def return_profile_complements(self, fam):
        ...
        losses = set(
            [n.name for n in tp.traverse() if n.lost and n.name in self.taxaIndex]
        )
    ...
    def retmat_mp(self, traindf, nworkers=25, chunksize=50):
        ...
        for i in range(nworkers):
            processes[i] = {
                "time": time.time(),
                "process": mp.Process(target=self.worker, args=(i, inq, retq)),
            }
        ...
        for batch in range(0, len(traindf), chunksize):
            ...
            fams = list(
                set(list(slicedf.HogFamA.unique()) + list(slicedf.HogFamB.unique()))
            )
            total = {}
            ...
            slicedf = slicedf.merge(
                retdf, left_on="HogFamA", right_index=True, how="left"
            )
            slicedf = slicedf.merge(
                retdf, left_on="HogFamB", right_index=True, how="left"
            )
    ...
    def hog_query(self, hog_id=None, fam_id=None, k=100):
        ...
        query_hash = hashutils.fam2hash_hdf5(
            fam_id, self.hashes_h5, nsamples=self.nsamples
        )
    ...
    def pull_hashes(self, hoglist):
        ...
        return {
            entry: hashutils.fam2hash_hdf5(
                self.hogid2fam(entry), self.hashes_h5, nsamples=self.nsamples
            )
    ...
    def allvall_nx(G, hashes, thresh=None):
        ...
        hashmat = [
            [
                hashes[hog1].jaccard(hashes[hog2]) if j > i else 0
                for j, hog2 in enumerate(hashes[0:i])
            ]
            for i, hog1 in enumerate(hashes)
        ]
