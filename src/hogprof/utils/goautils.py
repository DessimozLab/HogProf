...
def return_hogs_timeout(result, retq):
    print("started")
    with open_file(
        config_utils.config["dir"]["omadir"] + "OmaServer.h5", mode="r"
    ) as h5_oma:
...
def run_GOEA_onresults(results, db_obj, goeaobj, outname=None):
    ...
    if outname:
        ...
        goeaobj.wr_txt(
            config_utils.datadir + str(outname) + "enrichment.txt", goea_results_all
        )
    print("DONE!")
...
def get_go_terms_gaf(hog_id, pyoma_dbobj, gaf, genomes=None):
    ...
    go_terms = {
        mr.omaid: gaf[mr.omaid]
        for mr in pyoma_dbobj.iter_members_of_hog_id(hog_id)
        if mr.omaid in gaf
    }
    return go_terms
