...
def hash_tree(tp, taxaIndex, treeweights, wmg, lossonly=False, duplonly=False):
    ...
    losses = [
        taxaIndex[n.name] for n in tp.traverse() if n.lost and n.name in taxaIndex
    ]
    dupl = [taxaIndex[n.name] for n in tp.traverse() if n.dupl and n.name in taxaIndex]
    presence = [
        taxaIndex[n.name]
        for n in tp.traverse()
        if n.nbr_genes > 0 and n.name in taxaIndex
    ]
...
def row2hash(row, taxaIndex, treeweights, wmg, lossonly=False, duplonly=False):
    ...
    hog_matrix, weighted_hash = hash_tree(
        treemap, taxaIndex, treeweights, wmg, lossonly=lossonly, duplonly=duplonly
    )
