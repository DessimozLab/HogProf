    ...
    if orthoxml:
        ...
        try:
            ...
            if "maps to an ancestral name, not a leaf" in full_error_message:
                ...
                species = (
                    full_error_message.split("species name ")[1]
                    .split(" ")[0]
                    .replace("'", "")
                )
                if swap_ids == False and reformat_names == False:
                    species = " ".join(
                        full_error_message.split("species name ")[1].split(" ")[0:2]
                    ).replace("'", "")
                ...
                try:
                    ...
                    return get_ham_treemap_from_row(
                        row,
                        tree.write(format=1),
                        levels=levels,
                        swap_ids=swap_ids,
                        orthoXML_as_string=orthoXML_as_string,
                        use_phyloxml=use_phyloxml,
                        use_internal_name=use_internal_name,
